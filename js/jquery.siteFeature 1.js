...
(function($){
    ...
    $.fn.siteFeature.defaults = {
        ...
        containerHeight: '350px',                // sets the generated Container's height
        ...
        tabBgImg: 'images/left_arrow.png',        // relative path for Image used as Highlighted Tab background (ie: the arrow)
    ...
    $.fn.siteFeatureImgAnimations = function(opts, settings, hash) {
        ...
        if(opts.imgBgsAnimationType == 'wave'){    
            $.each(pieces, function(i){
                $(this).animate({'text-indent':'0'},timer,function(){
        ...
        if(opts.imgBgsAnimationType == 'fountain' || opts.imgBgsAnimationType == 'dribble'){
            $.each(order, function(i){
                pieces[this].animate({'text-indent':'0'},timer,function(){
                    $(this).animate({'opacity':'1','top':'0'},opts.imgBgsAnimationDuration,function(){
                        if((i + 1) == pieces.length){
                            timer = 0;
                            $target.css({'opacity':'1'}).show();
    ...
    function customizeFeature(opts,settings){
        ...
        */
        ...
        if(!opts.tabLock){
            ...
            if(opts.tabsLocation == 'left'){
                settings.tabTriggers.css({
                    'left' : obj[0].offsetLeft + ((parseInt(opts.containerWidth) - parseInt(opts.imgWidth)) / 2) - ((settings.tabTriggers.filter('#SF-n-prev').width() / 2)) // Mark it
                });
            } else {
                settings.tabTriggers.css({
        ...
        tabs.children().css({
            ...
            'left':( (opts.tabsLocation == 'left') ? parseInt(opts.containerWidth) - (parseInt(opts.imgWidth) : '0')) //Mark It
        }).prev().css({
            'width':(parseInt(opts.containerWidth) - parseInt(opts.imgWidth))
        });
 ...
 * https://github.com/kitajchuk/jQuery-backgroundPosition-for-Internet-Explorer
 */
(function ($) {
    if ( $.browser.msie ) {
